...
drumVolumes = (line 1.0, 0.5, steps: scales.length / 2).reflect

set :curScale, scales[0]


define :getRandomElement do |pauses, curScale|
  return [(knit :r, pauses, curScale.choose, 1).choose,
          (rrand 0.8, 1.2),
          (rrand -0.2, 0.2)]
end
...
define :liveLoop do |inst, sleepTime, amp, offset|
  ...
  live_loop ("loopMelody" + n.to_s).to_sym do
    ...
    idx = tick % len
    if one_in(get(:stop) ? 3 : randomness / sleepTime) or not melody[idx][0] in curScale
